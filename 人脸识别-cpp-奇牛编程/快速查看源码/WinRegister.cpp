    ...
    // 定时器2用于摄像头显示，并在3s时拍摄照片
        ...
        if (shoot_count < 3000 / 30) { // 3000表示3000ms(3s)，30是定时器2的中断间隔时间
            ...
            if (!faceRegion(faceModel, cam_frame, face_rect)) {
                ...
                int yanzhi = xishu * 100 + rand() % 25;           // 人脸相似度修正公式(防止相似度过低)
                yanzhi = (yanzhi < 0) ? 0 : yanzhi;               // 为负数时表示人脸比对失败
                yanzhi = (yanzhi > 100) ? 100 : yanzhi;           // 若超过100还是限定一下范围
                // 生成职业
                const char* jobs[] = { "教授", "副教授", "博士生", "研究生", "本科生" };
                // 获取编辑框内容
                CString edit_text;
                ...
                sprintf_s(user_filename, sizeof(user_filename), "users\\%s-%s-%d.jpg", jobs[rand() % 5], user_name, yanzhi);
